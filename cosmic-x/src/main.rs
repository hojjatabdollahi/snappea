...
fn main() -> cosmic::iced::Result {
    ...
    if args.len() > 1 && (args[1] == "--help" || args[1] == "-h") {
        println!("COSMIC X - Screenshot and Screen Recording for Linux/Wayland");
        println!();
        println!("Usage: cosmic-x [OPTION]");
        println!();
        println!("Options:");
    ...
    if args.len() > 1 && args[1] == "--record" {
        ...
        ) = (output_file, output_name, region, logical_size, encoder)
        {
            ...
            let state = recording::RecordingState {
            ...
            if let Err(e) = recording::start_recording(
                output_file,
                output_name,
                region,
                logical_size,
                ...
                let _ = recording::RecordingState::delete();
                std::process::exit(1);
            }

            // Clean up state file after successful recording
            if let Err(e) = recording::RecordingState::delete() {
    ...
    let instance_running = rt.block_on(dbus::is_instance_running());
    ...
    if instance_running {
        ...
        match rt.block_on(dbus::send_command(command)) {
            Ok(true) => {
            ...
            Err(e) => {
                log::warn!("Failed to send command: {e}, starting new instance");
    ...
    app::run_with_flags(app::AppFlags {
